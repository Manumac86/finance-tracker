...
export function FamilyMembersPanel({ familyGroup }: FamilyMembersPanelProps) {
  const { 
    ...
    error,
    removeMember 
  } = useFamilyMembers(familyGroup.organizationId);
  ...
  if (error) {
    return (
      <Card className="border-red-800 bg-red-900/10">
        <CardContent className="p-4">
          <p className="text-red-400 text-center">Failed to load family members</p>
          <p className="text-sm text-gray-500 text-center mt-1">{error.message}</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {members.map((member) => (
          <Card key={member.clerkUserId} className="border-gray-800 bg-gray-900/50">
            <CardContent className="p-4">
              <div className="flex items-center gap-4">
                {/* Avatar */}
        ...
        <Card className="border-gray-800 bg-gray-900/50">
          <CardContent className="p-8 text-center">
            <div className="text-gray-400 space-y-2">
              ...
              <p className="text-sm">Invite family members to start collaborating</p>
            </div>
          </CardContent>
        </Card>
      )}
